...
collector(void) {
    ...
    if (who == env_id_mat[0][0] && (value & PROCMASK) == PROCSTART) {
        x = (value & 0xFF00) >> 8;
        y = (value & 0XFF);
        cprintf("collector(%d, %d) %08x: started successfully.\n", x, y, thisenv->env_id);
    }
    else
        panic("collector: unexpected recv: %x got %x from %x\n", sys_getenvid(), value, who);

    // 进程主体
    cprintf("collector(%d, %d) %08x: collecting results.\n", x, y, thisenv->env_id);
    ...
    value = PROCEND;
    cprintf("collector(%d, %d) %08x: sending %d to master(%d, %d) %08x.\n", x, y, thisenv->env_id, value, 0, 0, env_id_mat[0][0]);
    ipc_send(env_id_mat[0][0], value, 0, 0);
    cprintf("collector(%d, %d) %08x: return successfully.\n", x, y, thisenv->env_id);
}
...
master(void) {
    ...
    cprintf("master(%d, %d) %08x: return successfully.\n", 0, 0, thisenv->env_id);
}
